...
class NeighborhoodAttention(nn.Module):
    ...
    def __init__(self, dim, kernel_size, num_heads,
        ...
        self.head_dim = dim // self.num_heads #64除以8
        ...
        self.rpb = nn.Parameter(torch.zeros(num_heads, (2 * kernel_size - 1), (2 * kernel_size - 1))) #8，13，13
    ...
    def forward(self, x):
        B, H, W, C = x.shape
        N = H * W #像素总大小
        num_tokens = int(self.kernel_size ** 2) #一个块是7*7
        ...
        qkv = self.qkv(x).reshape(B, H, W, 3, self.num_heads, self.head_dim).permute(3, 0, 4, 1, 2, 5) #8和8
        q, k, v = qkv[0], qkv[1], qkv[2]
        q = q * self.scale
        attn = NATTENQKRPBFunction.apply(q, k, self.rpb) #1，head,256,256,7*7
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        x = NATTENAVFunction.apply(attn, v) #1,head,256,256,C
        x = x.permute(0, 2, 3, 1, 4).reshape(B, H, W, C) #1，256，256，64
        if pad_r or pad_b:
            x = x[:, :Ho, :Wo, :]
