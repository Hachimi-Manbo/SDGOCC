...
def bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
    ...
    x = QuickCumsumCuda.apply(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                              ...
                              interval_lengths)      # (B, Dz, Dy, Dx, C)  每个三维点有C纬度特征然后属于那个柱子就相加
...
def test_bev_pool_v2():
    ...
    depth = depth.view(1, 1, 2, 2, 2).requires_grad_() #B, N, D, fH, fW
    feat = torch.ones(
        size=[1, 1, 2, 2, 4], dtype=torch.float,
        device='cuda').requires_grad_() # B, N, fH, fW, C
    ...
    bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                           (1, 1, 2, 2, 4), interval_starts, interval_lengths) # (B, Dz, Dy, Dx, C)
