...
from projects.mmdet3d_plugin.core.evaluation.eval_hooks import OccEvalHook
...
def train_detector(model,
    ...
    # prepare data loaders  到插件中查找：projects.mmdet3d_plugin.datasets.nuscenes_dataset_occ.NuScenesDatasetOccpancy
    dataset = dataset if isinstance(dataset, (list, tuple)) else [dataset]
    if 'imgs_per_gpu' in cfg.data: #flase
    ...
    data_loaders = [
        build_mmdet_dataloader(
            ...
            num_gpus=len(cfg.gpu_ids),#0  非分布式
            dist=distributed,
            seed=cfg.seed,
            runner_type=runner_type,
            persistent_workers=cfg.data.get('persistent_workers', False)) #flase
        for ds in dataset
    ]
    ...
    if distributed: #分布式
        ...
        model = MMDataParallel( #数据并行，模型初始化
            model.cuda(cfg.gpu_ids[0]), device_ids=cfg.gpu_ids)
    ...
    if 'runner' not in cfg:
        ...
        if 'total_epochs' in cfg: #只有max_epochs
