...
class FPN_LSS(nn.Module):
    ...
    def forward(self, feats):
        ...
        Returns:
            x: (B, C_out, 2*H, 2*W)
        """
        x2, x1 = feats[self.input_feature_index[0]], feats[self.input_feature_index[1]] #最上层与最下层
        if self.lateral:#flase
            x2 = self.lateral_conv(x2)
        x1 = self.up(x1)    # (B, C3, H, W) 双线性上采样
        x1 = torch.cat([x2, x1], dim=1)     # (B, C1+C3, H, W) cat
        x = self.conv(x1)   # (B, C', H, W)  3*3卷积
        if self.extra_upsample: #额外的上采样  上采样2倍
