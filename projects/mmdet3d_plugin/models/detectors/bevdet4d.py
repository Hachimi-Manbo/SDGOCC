...
class BEVDet4D(BEVDet):
    ...
    def prepare_inputs(self, img_inputs, stereo=False):
        ...
        N = N // self.num_frame     # N_views = 6  环视相机的数量
        imgs = img_inputs[0].view(B, N, self.num_frame, C, H, W) #顺序的三帧
        imgs = torch.split(imgs, 1, 2)
        imgs = [t.squeeze(2) for t in imgs]     # List[(B, N_views, C, H, W), (B, N_views, C, H, W), ...] 分成list
        ...
        keyego2global = ego2globals[:, 0, 0, ...].unsqueeze(1).unsqueeze(1)  #关键帧的全局坐标系？？？ 这六帧几乎相同
        ...
        if stereo: #true  0,1与1,2两帧的变换矩阵
            ...
            #全局坐标系是固定的坐标系。ego到全局的坐标系就是车辆在该时刻的位姿信息。
            # curr_sensor --> curr_ego --> global --> prev_ego --> prev_sensor先得到后两帧的车到全局求逆（后两帧全局到车）得到前两帧的车到全局，再得到前两帧的全局到车
            curr2adjsensor = \
                torch.inverse(ego2globals_adj @ sensor2egos_adj) \
                @ ego2globals_curr @ sensor2egos_curr       # (B, N_temporal=2, N_views, 4, 4)
            curr2adjsensor = curr2adjsensor.float()         # (B, N_temporal=2, N_views, 4, 4)  当前两帧帧到后两帧相机间的变换矩阵
            curr2adjsensor = torch.split(curr2adjsensor, 1, 1) #拆分成字典
            curr2adjsensor = [p.squeeze(1) for p in curr2adjsensor] #变成list
            curr2adjsensor.extend([None for _ in range(self.extra_ref_frames)])
            # curr2adjsensor: List[(B, N_views, 4, 4), (B, N_views, 4, 4), None] 两帧变换+NONE
        ...
        extra = [[p.squeeze(1) for p in t] for t in extra] #list，每个相机对应一个传感器关键车，车到全局，内参，数据增强矩阵
        ...
        return imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, \
               bda, curr2adjsensor   #每个相机对应一个传感器关键车，车到全局，内参，数据增强矩阵，BEV数据增强矩阵，当前帧到后两帧相机间的变换矩阵
