...
class NATBlock(nn.Module):
    ...
    def forward(self, x, x_multi):
        ...
        for cross_blk in self.cross_blocks:
            ...
            x_cross=checkpoint.checkpoint(cross_blk,x_multi,x_cross)
...
if __name__ == '__main__':
    nat_block = NATBlock(dim=256, depth=3, depth_cross=1, num_heads=8, kernel_size=7, downsample=False).cuda()
    x_in = torch.randn([1, 256, 256, 256]).cuda()
    x_multi = torch.randn([1, 256, 256, 256]).cuda()
