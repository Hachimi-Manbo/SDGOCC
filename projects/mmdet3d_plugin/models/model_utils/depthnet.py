...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            x: (B*N, C, fH, fW)
        """
        x1 = self.aspp1(x) #96通道
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
        x5 = self.global_avg_pool(x) #1*1
        x5 = F.interpolate(
            x5, size=x4.size()[2:], mode='bilinear', align_corners=True)
        x = torch.cat((x1, x2, x3, x4, x5), dim=1)  # (B*N, 5*C', fH, fW)  中间层为96通道

        x = self.conv1(x)   # (B*N, C, fH, fW) 256
        x = self.bn1(x)
        x = self.relu(x)
...
class DepthNet(nn.Module):
    def __init__(self,
        ...
        self.depth_mlp = Mlp(in_features=27, hidden_features=mid_channels, out_features=mid_channels) #256,256
        ...
        self.depth_head = TaskHead(mid_channels, mid_channels, depth_channels)
        self.semantic_head = TaskHead(mid_channels, mid_channels, 17)
        ...
        self.mtd = MultiTaskDistillationModule(mid_channels)
    ...
    def calculate_cost_volumn(self, metas,coor,mask):
        ...
        # D=9
        grid = self.gen_grid(metas, coor, mask, B, N, D, H, W, hi, wi).to(
            curr.dtype)  # (B*N_views, D*fH_stereo, fW_stereo, 2)  当前网格转换到之前帧的网格
    ...
    def forward(self, x, mlp_input, stereo_metas=None):
        ...
        """
        mlp_input = self.bn(mlp_input.reshape(-1, mlp_input.shape[-1]))     # (B*N_views, 27)  归一化
        x = self.reduce_conv(x)     # (B*N_views, C_mid, fH, fW)  3*3卷积

        # (B*N_views, 27) --> (B*N_views, C_mid) --> (B*N_views, C_mid, 1, 1)
        # context_se = self.context_mlp(mlp_input)[..., None, None] #升维
        # context = self.context_se(x, context_se)    # (B*N_views, C_mid, fH, fW)  通道注意力
        # context = self.context_conv(context)        # (B*N_views, C_context, fH, fW) 1*1卷积 通道不一样 64通道

        # (B*N_views, 27) --> (B*N_views, C_mid) --> (B*N_views, C_mid, 1, 1)
        depth_se = self.depth_mlp(mlp_input)[..., None, None]
        depth = self.depth_se(x, depth_se)      # (B*N_views, C_mid, fH, fW) 256通道

        if not stereo_metas is None:
            depth = self.aspp(depth)  # 总特征
            depth_feat = self.depth_conv(depth)
            semantic_feat = self.semantic_conv(depth)
            context = self.context_conv(depth)
            depth_feat, semantic_feat = self.mtd(depth_feat, semantic_feat)
            depth1 = self.depth_head(depth_feat, return_feat=False)  # 融合后的上采样特征
            semantic1 = self.semantic_head(semantic_feat, return_feat=False)
        else:
            depth = self.aspp(depth) #总特征
            depth_feat=self.depth_conv(depth)
            semantic_feat=self.semantic_conv(depth)
            ...
            depth1 = self.depth_head(depth_feat, return_feat=False)  # 融合后的上采样特征
            semantic1 = self.semantic_head(semantic_feat, return_feat=False)
        return torch.cat([depth1,semantic1, context], dim=1) #88+64=152
