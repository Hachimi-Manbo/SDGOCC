...
# def geo_scal_loss(pred, ssc_target, mask_camera, ignore_index=255, non_empty_idx=0):
#     pred = F.softmax(pred, dim=1)
#     empty_probs = pred[:, non_empty_idx]
#     nonempty_probs = 1 - empty_probs
#
#     # Remove unknown voxels
#     mask = (ssc_target != ignore_index) & (mask_camera)
...
#         )
#
# def sem_scal_loss(pred_, ssc_target, mask_camera, ignore_index=255):
#     with autocast(False):
...
def geo_scal_loss(pred, ssc_target, ignore_index=255, non_empty_idx=0):
    ...
    empty_probs = pred[:, non_empty_idx] #占用估计
    nonempty_probs = 1 - empty_probs

    # Remove unknown voxels
    mask = ssc_target != ignore_index   #标签255说明有占用但标签为0   无占用+标签不为0
    nonempty_target = ssc_target != non_empty_idx #非空的
    nonempty_target = nonempty_target[mask].float() #得到   0和1
    nonempty_probs = nonempty_probs[mask]
    empty_probs = empty_probs[mask]
