...
class Metric_mIoU():
    ...
    def compute_mIoU(self, pred, label, n_classes):
        ...
        return mIoUs,round(np.nanmean(mIoUs) * 100, 2), hist
    ...
    def add_batch(self, semantics_pred, semantics_gt, mask_camera):
        ...
        ious,_, _hist = self.compute_mIoU(masked_semantics_pred, masked_semantics_gt, self.num_classes) #计算MIOU的混淆矩阵
        self.hist += _hist    # (N_cls, N_cls)  列对应每个gt类别，行对应每个预测类别, 这样只有对角线位置上的预测是准确的.
        self.iou.append(ious)

    def count_miou(self):
        mIoU = self.per_class_iu(self.hist) #第17个为free voxel的iou
        ...
        print(f'===>voxel_occ - IoU = ' + str(round(mIoU[-1] * 100, 2)))
        return eval_res
