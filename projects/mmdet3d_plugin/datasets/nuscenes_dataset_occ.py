...
class NuScenesDatasetOccpancy(NuScenesDataset):
    def get_data_info(self, index):
        ...
        input_dict['occ_gt_path'] = self.data_infos[index]['occ_path']#occ_gt_path
        return input_dict

    # def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
    #     metric = eval_kwargs['metric']
    #     print("metric = ", metric)
    #     if metric == 'ray-iou':
    #         occ_gts = []
    #         occ_preds = []
    ...
    #             occ_gt = np.load(
    #                 os.path.join(info['occ_path'],'labels.npz'))
    ...
    # #     else:
    # #         self.occ_eval_metrics = Metric_mIoU(  #miou矩阵初始化
    # #             num_classes=18,
    # #             use_lidar_mask=False,
    # #             use_image_mask=True)
    # #
    # #         print('\nStarting Evaluation...')
    # #         for index, occ_pred in enumerate(tqdm(occ_results)):  #得到最后的混淆矩阵
    # #             # occ_pred: (Dx, Dy, Dz)
    ...
    # #                 os.path.join(info['occ_path'], 'labels.npz'))
    # #             gt_semantics = occ_gt['semantics']  # (Dx, Dy, Dz)
    # #             # gt_semantics[gt_semantics == 0] = 17
    # #             # gt_semantics[gt_semantics == 255] = 0
    # #             mask_camera = occ_gt['mask_camera'].astype(bool)  # (Dx, Dy, Dz)
    # #
    ...
    # #                 np.savez_compressed(save_path, pred=occ_pred, gt=gt_semantics, sample_token=sample_token)
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        ...
        for index, occ_pred in enumerate(tqdm(occ_results)):  #得到最后的混淆矩阵
            ...
            info = self.data_infos[index]
            # occ_gt = np.load(os.path.join(self.data_root, info['occ_path'], 'labels.npz'))
            occ_gt = np.load(os.path.join(info['occ_path'], 'labels.npz')) #GT
            gt_semantics = occ_gt['semantics']      # (Dx, Dy, Dz) 0到17
            mask_lidar = occ_gt['mask_lidar'].astype(bool)      # (Dx, Dy, Dz) 雷达掩码
            mask_camera = occ_gt['mask_camera'].astype(bool)    # (Dx, Dy, Dz) 相机掩码
            # occ_pred = occ_pred
            self.occ_eval_metrics.add_batch(
                occ_pred,   # (Dx, Dy, Dz)
                gt_semantics,   # (Dx, Dy, Dz)
    ...
    def vis_occ(self, semantics): #BEV结果可视化
