...
class NuScenesDatasetBEVDet(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        """
        info = self.data_infos[index] #得到kpl文件里的一个信息
        # standard protocol modified from SECOND.Pytorch
        input_dict = dict( #雷达点云的信息：token，路径，时间戳，sweeps（0）
        ...
        if 'ann_infos' in info:
            input_dict['ann_infos'] = info['ann_infos'] #标注信息
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv': #bevdet
                image_paths = []
                lidar2img_rts = []
                ...
                assert 'bevdet' in self.img_info_prototype   #true
                input_dict.update(dict(curr=info))
                if '4d' in self.img_info_prototype:     # 需要再读取历史帧的信息  #bevdet4d  历史帧信息
